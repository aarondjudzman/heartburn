...
export default function FoodSearch() {
  ...
  return (
    <div className="flex flex-col min-h-screen w-screen items-center bg-peach-yellow-50">
      <header className="sticky top-0 z-30 w-full flex h-20 bg-peach-yellow-50 items-center border-b-2 border-b-peach-yellow-800 px-4 shadow-lg">
        <div className="relative w-full flex justify-center">
          <Input
            type="search"
            placeholder="Search food..."
            onChange={(e) => handleSearch(e.target.value)}
            className="w-5/6 text-lg rounded-lg bg-transparent pl-2 border-2 border-peach-yellow-500 focus-visible:ring-peach-yellow-600"
          />
        </div>
      </header>
        ...
        <ScrollArea className="h-full p-4">
          {searchResults.length === 0 ? (
            ...
            searchResults.map((item, index) => (
              <Card
                key={index}
                className="mb-4 border-2 border-peach-yellow-400 shadow-sm"
              >
                <CardHeader>
                  <div className="flex justify-between items-center">
                    <div className="w-5/6">
                      <CardTitle className="text-xl mr-3 text-slate-600">
                        {item.name}
                      </CardTitle>
                    </div>
                    <div className="w-1/6">
